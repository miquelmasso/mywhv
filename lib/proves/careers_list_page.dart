...
class _CareersListPageState extends State<CareersListPage> {
  ...
  Future<void> _loadCareers() async {
    ...
    try {
      ...
      debugPrint('❌ Error loading careers pages: $e');
      if (!mounted) return;
      setState(() {
        _loading = false;
        _hasLoaded = true;
  ...
  Future<void> _exportCsv() async {
    ...
    try {
      final filePath = await exportRowsAsCsv(
        filePrefix: 'restaurants_job_pages',
        headers: const ['doc_id', 'name', 'careers_page'],
        rows: _restaurants
            .map(
              (row) => [
                (row['docId'] ?? '').toString(),
                (row['name'] ?? '').toString(),
                (row['careers_page'] ?? '').toString(),
              ],
            )
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'CSV creat (${_restaurants.length} job pages). Path copiat: $filePath',
          ),
        ),
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Restaurant careers pages'),
        backgroundColor: Colors.green,
        actions: [
          IconButton(
            tooltip: 'Reload',
            onPressed: _loading || _exporting ? null : _loadCareers,
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      final link = (r['careers_page'] ?? '').toString();
                      return Card(
