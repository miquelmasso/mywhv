...
class _OnboardingCard extends StatelessWidget {
  ...
  String _fallback(Map<String, String> strings, String key, String fallback) {
    final translated = strings[key];
    if (translated == null || translated.isEmpty) return fallback;
    return translated;
  }

  List<String> _translatedBullets(Map<String, String> strings) {
    return List<String>.generate(step.bullets.length, (index) {
      return _fallback(
        strings,
        'onboarding.${step.id}.bullet.$index',
        step.bullets[index],
      );
    });
  }
    ...
    return FutureBuilder<Map<String, String>>(
      ...
      builder: (context, snapshot) {
        ...
        final title = _fallback(
          strings,
          'onboarding.${step.id}.title',
          step.title,
        );
        final description = _fallback(
          strings,
          'onboarding.${step.id}.description',
          step.description,
        );
        final primaryLabel = _fallback(
          strings,
          'onboarding.${step.id}.primary',
          step.primaryLabel,
        );
        final bullets = _translatedBullets(strings);
        final skipLabel = AppI18n.t(strings, 'onboarding.skip');
